...
- 解凍したフォルダ内の「etl-template」フォルダにテンプレートファイルがあります。

## テンプレート
ETLの定型的な処理フローをテンプレートとして提供しています。テンプレートをベースにすることで、バッチを効率的に作成できます。
...
## 図形の操作
- 配置/追加
    - 画面左のパレットからドラッグ＆ドロップで図形を配置できます。
    - 図形を選択した際に図形の右に表示されるメニューからドラッグ＆ドロップで図形を追加できます。この方法で追加した図形は、矢印で繋がれた状態で生成されます。  
    ![context-pad](image/context-pad.png)
    - 図形選択時に右に表示されるメニューは画面を一定以上縮小すると表示されなくなります。
...
- 削除
    - 図形を選択して削除ボタンをクリック、またはキーボードのdeleteキーを押すことで削除できます。  
...
- キャンバスの移動
    ...
    - マウスホイールでキャンバスを上下させることができます。
    - Shift + マウスホイールでキャンバスを左右させることができます。

- キャンバスの拡大・縮小
    - Ctrl + マウスホイールでキャンバスの拡大・縮小ができます。
    - Ctrl + + で拡大、Ctrl + - で縮小ができます。

- 図形の検索
    - キャンバス上の図形を検索できます。
    - Ctrl + f で検索ボックスが表示されます。
    - 検索ボックスに入力した文字列に部分一致する図形が画面中心になり、ハイライト表示されます。
    - 検索対象は図形の種類(Step、Listenerなど)と、プロパティパネルのname属性に設定した値になります。
    - 図形が大きく画面外に移動してしまった際に、この機能で図形を画面内に戻すことができます。
...
- エラー： ジョブを実行するとエラーになるもの
    - 必須項目の未設定
    - ジョブのname属性が未設定
    - ステップのname属性が重複している
- 警告： エラーにはならないが、修正すべきもの
    - ジョブレベルリスナーが未設定
    - ステップのname属性が未設定
...
- 変換する際には、Jobのname属性を設定する必要があります。
  - Jobのname属性を設定していない状態で変換を選択すると、ダイアログが表示され変換できません。
  - ETLデザイナのキャンバスで図形を選ばない状態で、プロパティパネルの「General」を選択し、「name」属性を入力します。
  
  ![export-xml](image/export-xml.png)
  
- 変換する際は、編集内容をファイルに保存する必要があります。
  - 編集内容を保存していない状態で変換を選択すると、ダイアログが表示され変換できません。
  - ファイル＞上書き保存、もしくは名前を付けて保存で編集内容を保存してください。
...
- ※設定ファイルは、windows標準のメモ帳で開くとレイアウトが崩れます。改行コードが設定できる別のテキストエディタを使用してください。(サクラエディタなど)
- ※設定ファイルがjson形式の文法に沿っていない場合、ETLデザイナーが正常に動作しません。編集する際はjson形式の文法をチェックできるエディタやIDEの使用を推奨します。
- 3種類の設定ファイルがあります。
    - registry.json：以下の設定ファイルのパス
    - appConfig.json：ETLデザイナー自体の設定（言語設定など）
